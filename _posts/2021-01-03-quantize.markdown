...
其中，V_x表示原始浮点输入，V_quant表示量化后的定点数值，V_quant_revert是根据量化参数还原出的浮点数，quant_bits表示量化位数（1~32）。假设 RQM = round(Q * V_min)，那么上述公式可以进一步简化为：

```python
V_quant = round(Q * V_x) - RQM
V_quant_revert = (V_quant + RQM) / Q
```
